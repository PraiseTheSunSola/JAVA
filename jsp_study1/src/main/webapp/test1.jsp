...
<%--
	...
	<%@ -jsp 지시어 스크립트 작성 부분
	지시어는 해당 jsp 페이지의 설정 하기 위한 부분이다. 
	...
	page - jsp페이지의 속성을 지정하는 역활
	       ...
	       autoFlush : 출력버퍼가 초과되면 수행할 기능지정
	       			   ture일 경우 버퍼가 100%이면 내용을 화면에 출력
	       			   flase일 경우 IOException 발생
	       예외처리에서 throw를 하는 이유와 같이 기록(log)을 남겨두기 위해 
	       false로 사용 하기도 한다. 
	       
	       
	include - 다른 페이지에 있는 html, css, 자바스크립트 내용을 가져온다. 
			  다른 jsp 페이지를 추가하기 ( 현재페이지에 내용을 포함)
	...
	클라이언트는 서버로부터 받은 내용을 브라우저가 실행하며 화면에 출력.
